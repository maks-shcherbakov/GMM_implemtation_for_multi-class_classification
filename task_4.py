...
trained_GMM_file_1 = 'data/GMM_params_phoneme_01_k_03.npy'
trained_GMM_file_2 = 'data/GMM_params_phoneme_02_k_03.npy'
...
grid1=np.arange(min_f1,max_f1)
grid2=np.arange(min_f2,max_f2)
for k in range(N_f2):
    print(k)
    for l in range(N_f1):
        grid=np.array([[grid1[l],grid2[k]]])
        prediction1=get_predictions(mu1,s1,p1,grid)
        prediction2=get_predictions(mu2,s2,p2,grid)
        if(np.mean(prediction1)>np.mean(prediction2)):
            M[k][l]=0.0
        else:
            M[k][l]=1.0
################################################
